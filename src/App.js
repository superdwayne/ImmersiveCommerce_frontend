...
function Model({ url }) {
  const { scene, progress } = useGLTF(url, (state) => {
    // This function is called during loading, so we can set the loading progress here
    if (state.loading) {
...
export default function Component() {
  ...
  <Suspense fallback={<Center><mesh><boxGeometry args={[1, 1, 1]} /></mesh></Center>}>
    {currentProduct && currentProduct.glbUrl ? (
      <Model url={currentProduct.glbUrl} position={[0, 0, 0]} /> 
    ) : (
      <Center>
        <mesh>
  ...
  <OrbitControls target={[0, 0, 0]} /> {/* Ensure controls target center */}
</Canvas>
